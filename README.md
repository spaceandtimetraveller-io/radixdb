...
- **I/O model**: Persistence uses a normal `*os.File` with **`ReadAt` / `WriteAt`** and **`Truncate`** for growth. **`blockMgr`** keeps an **LRU** of whole blocks (default capacity `DefaultBlockCacheEntries`, 4 KiB each) and a **`sync.Pool`** to reuse block-sized buffers on load. Evicted cache pages are not returned to the pool while any `readBlock` slice might still alias them. There is **no `mmap`**. Node prefixes and row paths are **copied** out of block bytes so eviction cannot corrupt decoded data. In-place bump allocation uses **`modifyBlock`** (single critical section) so a block buffer cannot be stolen between read and write.
